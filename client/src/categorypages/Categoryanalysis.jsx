  ...
  const fetchData = async () => {
    try {
      ...
      data.forEach(order => {
        ...
        ordersCountByDate[date] = ordersCountByDate[date] ? ordersCountByDate[date] + 1 : 1;
      });
    ...
    datasets: [
      {
        ...
        data: Object.keys(ordersData.reduce((acc, curr) => {
          ...
          return acc;
        }, {})).map(date => ordersData.reduce((acc, curr) => {
          if (curr.date === date) acc++;
          return acc;
        }, 0)),
        fill: false,
            ...
            <div className='main-box'>
              ...
              <ul>
                {topDates.map((date, index) => (
                  <li key={index}>{date}: {ordersData.filter(item => item.date === date).length} products</li>
                ))}
              </ul>
            </div>
          </div>
